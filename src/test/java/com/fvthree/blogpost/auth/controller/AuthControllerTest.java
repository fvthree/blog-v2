...
class AuthControllerTest {
	...
	void testLogin() throws Exception {
		...
		when(authService.login(new LoginRequest(created.getUsername(), created.getPassword())))
	...
	void testRegister() throws Exception {
		...
		when(authService.register(created.getUsername(), created.getEmail(), created.getPassword()))
			.thenReturn(created);
	...
	void testGetProfile() throws Exception {
		...
		when(jwtTokenProvider.getUsername(any(String.class))).thenReturn(created.getUsername());
		when(userDao.findById(any(Long.class))).thenReturn(created);
		
		String profileId = "1";
		
		this.mockMvc.perform(get("/api/v1/users/profile/" + profileId)
				.header("Authorization",  "Bearer " + created.getUsername()))
				.andDo(print())
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.username").value("username"))
				.andExpect(jsonPath("$.email").value("email@email.com"));
