...
public class AuthServiceImpl implements AuthService {
	...
	public String login(LoginRequest loginRequest) {
		logger.info("login email = {} ", loginRequest.getUsername());
		try {
			String encodedPassword = passwordUtil.hashPassword(loginRequest.getPassword());
			logger.info("password = {}", encodedPassword);
			User user = userDao.findByEmailOrUserAndPassword(loginRequest.getUsername(), encodedPassword)
					.orElseThrow(()-> new RestAPIException("invalid credentials"));
			var userDetails = new UsernamePasswordAuthenticationToken(loginRequest.getUsername(), loginRequest.getPassword());
